...
cd $PROGRESS_DIR
echo '1/24: boot Debian' > $PROGRESS_PATH
...
echo '2/24: change all files datetime to epoch' > $PROGRESS_PATH
...
echo '3/24: download the gentoo stage tarball' > $PROGRESS_PATH
cd /tmp
...
echo '4/24: unpack the stage tarball' > $PROGRESS_PATH
tar xvjpf stage3-*.tar.bz2 --xattrs --numeric-owner -C /

# move lib dirs temporary, and regen symlinks
echo '5/24: move lib dirs temporary, and regen symlinks' > $PROGRESS_PATH
...
echo '7/24: install an ebuild repository snapshot from the web' > $PROGRESS_PATH
emerge-webrsync
...
# configure locales
echo '8/24: configure locales' > $PROGRESS_PATH
...
echo '9/24: configure the network' > $PROGRESS_PATH
...
echo '10/24: install networking tools (dhcpcd)' > $PROGRESS_PATH
emerge net-misc/dhcpcd

# remove old setting files
echo '11/24: remove old setting files' > $PROGRESS_PATH
